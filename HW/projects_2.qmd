...
2. Your goal here is to find conditions that a vector $\mathbf{b}$ must satisfy in order for it to be a vector of voltages for some unknown distribution of potentials on the nodes. Use the fact (p. 422) that $A \mathbf{x}=\mathbf{b}$ implies that for all $\mathbf{y} \in \mathcal{N}\left(A^{T}\right), \mathbf{y}^{T} \mathbf{b}=0$. 
