...
<!-- @idea: eigenvalues_eigenspaces_algorithm | Eigenvalues, eigenspaces, characteristic equation -->
<!-- @uses: diagonalization, eigenvalue, eigenvector, eigenspace, eigenvalue_eigenvector -->
Definitions, how to find them, and the eigensystem algorithm.

<!-- @introduces: eigenvalue -->
<!-- @introduces: eigenvector -->
## Eigenvalues and Eigenvectors
...
::: definition
- **Algebraic multiplicity** of $\lambda$: multiplicity of $\lambda$ as a root of the characteristic equation.
- **Geometric multiplicity** of $\lambda$: dimension of the space $\mathcal{E}_{\lambda}(A)=\mathcal{N}(\lambda I-A)$.
:::

. . .

::: definition
- **Simple**: algebraic multiplicity is 1.
- **Repeated**: algebraic multiplicity $> 1$.
:::
...
- Compute the characteristic polynomial and find eigenvalues of a matrix
- Find eigenvectors and determine an eigensystem
- State the algebraic and geometric multiplicity of an eigenvalue

# Similarity and Diagonalization

Using eigenvectors to simplify matrix functions.

<!-- @idea: diagonalization_and_similarity | Diagonalization, similar matrices -->
...
$$
\begin{aligned}
\sin \left(\frac{\pi}{2} A\right)&=P \sin \left(\frac{\pi}{2} D\right) P^{-1} \\[20pt]
...
- Diagonalize a matrix and compute $P$, $D$, and $P^{-1}$
- Compute matrix functions via diagonalization

# Application to Discrete Dynamical Systems

<!-- @idea: spectral_radius_and_dynamics | Spectral radius, dominant eigenvalue, dynamics -->
<!-- @uses: dominant_eigenvalue, eigenvalue, eigenvalue_eigenvector, spectral_radius -->
How eigenvalues govern long-run behavior of iterated systems.
...
<!-- @introduces: spectral_radius -->
<!-- @introduces: dominant_eigenvalue -->
...
![Fibonacci Statue](images/day_11a_fibonacci.jpg){height=200}
...
(Treatment from [here](https://www.math.umd.edu/~immortal/MATH401/book/ch_discrete_dynamical_systems.pdf))
...
Eigenvalues (call them phi and psi):
$$
\begin{aligned}
...
$$
$$
=\frac{1}{\sqrt{5}}\left[\begin{array}{c}
\varphi^{k}-\psi^{k} \\
\varphi^{k+1}-\psi^{k+1}
\end{array}\right]
$$
...
![Golden Ratio spiral](https://upload.wikimedia.org/wikipedia/commons/b/b9/Fibonacci_Spiral.svg)
:::


## Skills

- Express an initial state as a linear combination of eigenvectors
- Predict long-run behavior using the spectral radius and dominant eigenvalue
- Apply eigenvalue analysis to discrete recurrences (e.g., Fibonacci)
